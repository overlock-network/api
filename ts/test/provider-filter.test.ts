...
it('provider-filter', async () => {
  ...
  for (const [index, accountData] of accounts.entries()) {
    ...
    if (account) {
      const message = overlock.crossplane.MessageComposer.fromPartial.createProvider({
      ...
      let id: string = "-1"
      response.events.filter((event: Event)=>event.type == "provider-created")
        .forEach((event)=>{
